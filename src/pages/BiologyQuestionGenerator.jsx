...
const BiologyQuestionGenerator = () => {
  ...
  const biologyTopics = [
    {
      title: t("biology.topic.cellularMolecular"),
      apiName: "Biologia Celular y Molecular",
    },
    { title: t("biology.topic.genetics"), apiName: "Genetica" },
  ];
  const selectedTopic =
    biologyTopics.find((topic) => topic.apiName === selectedTopicApiName) || null;

  const generateQuestion = async (topic) => {
    try {
      setIsLoading(true);
      setErrorMessage("");
      const response = await post("ai/biology", { parameter: topic.apiName });
      resetQuestionState();
      resetQuestionData();
...
export default BiologyQuestionGenerator;
